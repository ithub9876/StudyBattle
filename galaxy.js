...
export function initGalaxy(container, opts = {}) {
  const testCanvas = document.createElement('canvas');
  const testGl = testCanvas.getContext('webgl') || testCanvas.getContext('experimental-webgl');
  if (!testGl) {
    container.style.cssText = 'position:fixed;inset:0;z-index:0;background:radial-gradient(ellipse at 50% 0%,#0d0d2b 0%,#000005 60%,#000 100%);pointer-events:none;';
    return () => {};
  }
  const {
    focal = [0.5, 0.5],
  ...
  function update(t) {
    ...
    const lr = 0.05;
    smoothMouse.x += (targetMouse.x - smoothMouse.x) * lr;
    smoothMouse.y += (targetMouse.y - smoothMouse.y) * lr;
    smoothActive += (targetActive - smoothActive) * lr;
